...
sys.path.insert(0, str(Path(__file__).parent.parent))  # reach providers.py
...
NARRATIVE_MD = ROOT / "results" / "NARRATIVE.md"
...
NARA_SYSTEM = """You are NARA — the research intelligence embedded in this CMP architecture research session.
You have full access to all experimental results, the research narrative, the session log, and the research constitution.
You are a neuroscientist and AI researcher who thinks in mechanisms, not vague directions.
Answer questions about the research precisely and concisely. Separate PROVEN from HYPOTHESIS from SPECULATION.
Keep responses under 200 words unless the user explicitly asks for a detailed breakdown.
Never hallucinate results — only cite what is in the provided context."""
...
def narrative():
    if not NARRATIVE_MD.exists():
        return jsonify({"text": None})
    return jsonify({"text": NARRATIVE_MD.read_text(errors="replace")})
...
def chat():
    ...
    if NARRATIVE_MD.exists():
        ctx_parts.append("## Research Narrative\n" + NARRATIVE_MD.read_text(errors="replace")[:3000])

    if AGENT_MD.exists():
        ctx_parts.append("## Research Constitution (AGENT.md)\n" + AGENT_MD.read_text(errors="replace")[:2000])

    if SESSION_LOG.exists():
        log_tail = "\n".join(_read_tail(SESSION_LOG, 40))
        ctx_parts.append("## Recent Session Log\n" + log_tail)

    rows = _parse_results()
    if rows:
        ctx_parts.append("## Last Experiments (newest first)\n" + str(rows[-10:]))

    if CONCLUSION.exists():
        ctx_parts.append("## Conclusion (if written)\n" + CONCLUSION.read_text(errors="replace")[:1000])
    ...
    try:
        if not os.environ.get("GEMINI_API_KEY", ""):
            return jsonify({"reply": "(NARA offline — GEMINI_API_KEY not set)"})
        client = make_client("gemini", GEMINI_MODEL, use_search=False)
        response = client.generate_content(full_prompt, system=NARA_SYSTEM)
        return jsonify({"reply": response.text.strip()})
    except Exception as e:
        return jsonify({"reply": f"(NARA error: {e})"})
