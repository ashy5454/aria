...
from paper_reader import search_papers, format_for_council
from narrator import run_narrator
from challenger import challenge_hypothesis
...
EVOLVE_EVERY    = _loop.get("evolve_every", 5)
NARRATOR_EVERY  = _loop.get("narrator_every", 5)
...
NARRATIVE_MD = RESULTS_DIR / "NARRATIVE.md"
...
def main():
    ...
    last_web_query = ""                       # fed to paper_reader next experiment
    ...
    while True:
        ...
        # ── LITERATURE AGENT — fetch papers for current search direction ──
        lit_context = ""
        if last_web_query:
            try:
                cards = search_papers(last_web_query, max_papers=4)
                lit_context = format_for_council(cards)
            except Exception as e:
                log(f"[lit] paper_reader failed: {e}")

        # ── AGENT 1: PLANNER (or Council) ─────────────────────────────────
        constitution_text = AGENT_MD.read_text() if AGENT_MD.exists() else ""
        challenger_concerns = ""
        plan = {}

        for council_attempt in range(2):   # max 1 re-run after challenger rejection
            if _COUNCIL_AVAILABLE:
                if council_attempt == 0:
                    log("[1/4] HYPOTHESIS COUNCIL — 5 personas + chairman...")
                else:
                    log("[1/4] COUNCIL RE-RUN — challenger rejected, injecting concerns...")
                plan = run_hypothesis_council(
                    ...
                    challenger_concerns=challenger_concerns,
                )
            ...
            # ── CHALLENGER — stress-test before coding ──────────────────
            log("[1b] CHALLENGER — reviewing hypothesis...")
            challenge = challenge_hypothesis(
                hypothesis=plan.get("hypothesis", ""),
                mechanism_chain=plan.get("mechanism_chain", ""),
                predicted_delta=plan.get("predicted_delta", ""),
                rationale=plan.get("rationale", ""),
                recent_results=load_recent_results(10),
                hard_constraints=constitution_text[:1500],
                gemini_model=GEMINI_MODEL,
                provider=PROVIDER,
            )
            verdict = challenge.get("verdict", "APPROVED")
            if verdict == "REJECTED" and council_attempt == 0:
                reasons = [
                    challenge.get("constraint_violation") or "",
                    challenge.get("causal_chain_flaw") or "",
                    challenge.get("repeat_detected") or "",
                    challenge.get("rejection_reason") or "",
                ]
                challenger_concerns = "\n".join(r for r in reasons if r)
                log(f"[challenger] REJECTED — re-running council with concerns")
                continue

            if challenge.get("strengthened_hypothesis"):
                plan["hypothesis"] = challenge["strengthened_hypothesis"]
            break
        ...
        last_web_query = web_q or last_web_query   # carry forward for next experiment's paper search
        log(f"[planner] Hypothesis: {hypothesis}")
        log(f"[planner] Chain: {plan.get('mechanism_chain','')[:80]}")
        ...
        # ── NARRATOR — living research narrative ────────────────────────
        if experiment_n % NARRATOR_EVERY == 0:
            log(f"[narrator] Synthesizing research narrative at experiment #{experiment_n}...")
            try:
                skill_files = sorted(mem.SKILLS_DIR.glob("*.md")) if mem.SKILLS_DIR else []
                skill_summaries = [sf.read_text()[:300] for sf in skill_files[-15:]]
                run_narrator(
                    results_tsv=RESULTS_TSV.read_text() if RESULTS_TSV.exists() else "",
                    session_log_tail="\n".join(
                        (SESSION_LOG.read_text().splitlines()[-30:]) if SESSION_LOG.exists() else []
                    ),
                    agent_md=AGENT_MD.read_text() if AGENT_MD.exists() else "",
                    domain_context=DOMAIN_CONTEXT,
                    skill_summaries=skill_summaries,
                    best_metric=best_metric,
                    metric_goal=METRIC_GOAL,
                    experiment_n=experiment_n,
                    output_path=NARRATIVE_MD,
                    gemini_model=GEMINI_MODEL,
                    provider=PROVIDER,
                )
            except Exception as e:
                log(f"[narrator] failed: {e}")
